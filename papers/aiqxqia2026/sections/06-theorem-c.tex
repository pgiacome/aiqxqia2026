...
\begin{theorem}[Path states]
  ...
  this way, and any two realisations differ by a global isometry together with a phase
  on each point. The correspondence $f \leftrightarrow \Phi_f$ is thus a bijection onto
  the strictly monotone ultrametric kernels.
\end{theorem}
...
\begin{proof}
  ...
  For the converse, suppose $K$ is ultrametric with strictly monotone profile $f$ and
  that $\Psi$ is any realisation. Then $\abs{\braket{\Psi(x)}{\Psi(y)}} = \sqrt{f(\lcadepth)}$,
  which by \cref{eq:overlap} equals $\abs{\braket{\pathstate{x}}{\pathstate{y}}}$. Two
  families of unit vectors with equal pairwise overlap moduli agree up to a phase
  $e^{i\vartheta_x}$ on each point and a global isometry of the span, by the standard
  Gram-matrix argument. Conversely $a_i^{2} = \sqrt{f(i)} - \sqrt{f(i-1)}$ is forced by
  \cref{eq:overlap}, so $f$ determines $\Phi_f$ up to that freedom.
\end{proof}

\begin{remark}[Uniqueness is up to isometry, not relabelling]
  \label{rem:uniqueness}
  It is tempting to state the converse as uniqueness up to relabelling of $V$. That is
  false: rotating every $\pathstate{x}$ by a fixed unitary, or multiplying each by its
  own phase, leaves the fidelity kernel unchanged while destroying the interpretation
  of the basis as tree nodes
...
Critically, it has only $n+1$ non-zero amplitudes out of $\abs{V}$: it is a sparse
state, and sparse state preparation with $n+1$ non-zeros costs
$O(n \log \abs{V})$ elementary gates. In a level-indexed layout, where the register is
split into a depth counter and a within-level index, preparation is a cascade of $O(n)$
controlled rotations, one per level, each splitting the remaining amplitude between
``stop here'' and ``descend''.
...
On the regular $p$-ary tree, $\lcadepth(x,y) = \vp(x-y)$ and the profile can be tied
directly to the $p$-adic absolute value by taking $f(v) = p^{-(n-v)s} = \abs{x-y}_p^{s}$
for a scale $s > 0$. The path state is then a level-weighted superposition over the
