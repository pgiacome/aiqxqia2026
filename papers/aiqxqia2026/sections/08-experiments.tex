...
We then truncate every hierarchy at depth $8$. This is needed because the
...
$2^{\EOneSourceHeight}$ amplitudes per point --- and it has the further benefit that
every encoding is compared on exactly the same tree. Finally we sample $256$ leaves,
...
the profile base to the branching factor instead --- $38$ for NCBI --- pushes
...
$\RandProdLevelConstancy$ --- so their kernels are not merely imperfect ultrametrics
...
representational capacity on depths $3$ through $8$ is spending it on distinctions the
